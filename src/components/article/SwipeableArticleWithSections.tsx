...
const SwipeableArticleWithSections = (props: SwipeableArticleWithSectionsProps) => {
  ...
  return <div ref={constraintsRef} className="article-section h-screen w-screen snap-start snap-always relative flex items-center justify-center overflow-hidden" data-index={props.index}>
      <motion.div className="w-full h-full relative" drag={isWikipediaArticle && totalSections > 1 ? "x" : false} dragConstraints={constraintsRef} dragElastic={0.2} onDragStart={() => setIsDragging(true)} onDragEnd={handleDragEnd} whileDrag={{
      cursor: "grabbing"
    }} {...getAnimationVariants()}>
        {/* Background Image with smooth transition */}
        <motion.div key={`${props.article.id}-${currentSection}`} className="absolute inset-0 w-screen h-screen" initial={swipeDirection ? {
        opacity: 0.8
      } : false} animate={{
        opacity: 1
      }} transition={{
        duration: 0.3
          ...
          <img src={getCurrentImage()} alt={getCurrentTitle()} className="w-full h-full object-cover" loading={props.index <= props.currentIndex + 1 ? "eager" : "lazy"} />
          <div className="absolute inset-0 bg-gradient-to-b from-black/30 via-transparent to-black/90" />
        </motion.div>

        {/* Swipe hint using loading popup style */}
        {showSwipeHint && isWikipediaArticle && <div className="hidden absolute top-20 right-4 z-30 bg-black/50 backdrop-blur-sm text-white px-3 py-2 rounded-full text-sm flex items-center gap-2">
            <div className="w-2 h-2 bg-white rounded-full animate-pulse"></div>
            Swipe for more content
          </div>}

        {/* Loading indicator for sections */}
        {isLoadingSections && <div className="absolute top-20 right-4 z-30 bg-black/50 backdrop-blur-sm text-white px-2 py-1 rounded-full text-xs flex items-center gap-1">
            <div className="w-2 h-2 bg-white rounded-full animate-pulse"></div>
            Loading...
          </div>}

        <ArticleDisplay {...props} article={{
        ...props.article,
        ...
        {isWikipediaArticle && totalSections > 1 && <div className="absolute bottom-20 left-1/2 transform -translate-x-1/2 z-30 flex space-x-2">
            {Array.from({
          length: totalSections
        }).map((_, index) => <div key={index} className={`w-2 h-2 rounded-full transition-all duration-300 ${index === currentSection ? 'bg-white scale-110' : 'bg-white/40 scale-100'}`} />)}
            {/* Show loading dot if more sections are being loaded */}
            {isLoadingSections && <div className="w-2 h-2 rounded-full bg-white/20 animate-pulse scale-90" />}
          </div>}
      </motion.div>
    </div>;
};
